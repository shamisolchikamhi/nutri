...
test("opens an action-focused Nutri Agent entry point", async ({ page }) => {
  ...
  await expect(page.getByText("Safety boundaries")).toBeVisible();
  await expect(page.getByText("High-risk goals are escalated")).toBeVisible();
  await expect(page.getByText("Aggressive deficits are blocked")).toBeVisible();
  await expect(page.getByText("Every recommendation should show what is known, estimated, missing, or stale.")).toBeVisible();
